...
Sessions message each other by writing small files under `~/.claude/ccc/mail/`. By default Claude Code prompts for permission on each such write. The Grant button adds two narrowly-scoped rules — `Edit(~/.claude/ccc/mail/**)` and `Edit(~/.claude/ccc/mail-dev/**)` — to your global `~/.claude/settings.json` (backed up first) so those writes stop prompting. They are scoped to the mail trees deliberately, not `~/.claude/ccc/**`: that tree also holds the status-hook script that runs on every hook event, and a session is not pre-authorized to edit it. Once granted, the button reads "Granted ✓" and is disabled. This edits your global Claude settings; you can remove the rule there by hand at any time. There is also a first-run dialog that offers the same grant; declining it just records that you have seen it, and you can grant later here.
